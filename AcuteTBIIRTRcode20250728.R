...
# from clinical, neuroimaging, and blood-based biomarkers 
...
dat$CompleteDataLOCPTA <- ifelse(complete.cases(dat[, c(
  "LOCDuration_R",
  "LOCPTADuration_R"
)]), 1, 0)
table(dat$CompleteDataLOCPTA)
...
categorical_vars <- c(
  "Sex", "Race_r", "Ethnicity", "HealthInsurance_r",
  "ctpos", "NumberNonreactingPupils", "InjCause_r",
  "levcare", "LOCDuration_r", "LOCPTADuration_r")
...
ModelMissingLOCPTA <- glm(
  CompleteDataLOCPTA ~ Age + Sex + EduYearsOfEducation +
    gcser + ctpos + InjCause_r + levcare,
  data = dat,
  family = binomial(link = "logit")
)
summary(ModelMissingLOCPTA)
anova(ModelMissingLOCPTA, test = "Chisq")
exp(cbind(OR = coef(ModelMissingLOCPTA), confint(ModelMissingLOCPTA)))
